...
{
    x = x>kDeviceWidth?kDeviceWidth:x;
    x = x<0?0:x;
    ...
    float scale = (x/(kDeviceWidth/(1-0.5))) + 0.5;
    float alpha = 0.4 - 0.4*(x/kDeviceWidth);
    
    lastScreenShotView.transform = CGAffineTransformTranslate(CGAffineTransformIdentity, -kDeviceWidth * (1 -scale), 0);//CGAffineTransformMakeScale(scale, scale);
